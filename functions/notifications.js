...
let storeToken = async (userId) => {
    ...
    var tokenRef = await firebase.database().ref("users/" + userId + "/pushTokens");
...
let sendChatNotificationMessage = async (userId, messageContent) => {
    var userRef = await (await firebase.database().ref("users/" + userId).get()).val();
    var userName = userRef.firstname.concat(" ", userRef.lastname);
    
    var tokens = await firebase.database().ref("users/" + userId + "/pushTokens").get();
